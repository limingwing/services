...
public class submit_ajax : IHttpHandler, IRequiresSessionState
{
    ...
    private void cart_goods_add(HttpContext context)
    {
        ...
        //统计购物车
        ShopCart.Add(goods_id + '|'+"暂无", goods_quantity);
    ...
    private void cart_goods_update(HttpContext context)
    {
        ...
        p.GetModel(Convert.ToInt32(goods_id.Split('|')[0]));
        if (p.pro_num < goods_quantity)
        {
            context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品库存不足，请重新选择数量！\"}");
            return;
        }
