...
using System.Data.SqlClient;
    ...
    /// </summary>
    [Serializable]
    public partial class tbl_sinkia_class
    {
        public tbl_sinkia_class()
        { }
        #region Model
        private int _id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public tbl_sinkia_class(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,pro_class,pid,type,flag,sort_id ");
            strSql.Append(" FROM [tbl_sinkia_class] ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from [tbl_sinkia_class]");
            strSql.Append(" where id=@id ");
        ...
        /// 查询名称是否存在
        /// </summary>
        public bool Exists(string pro_class)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from  tbl_sinkia_class");
            strSql.Append(" where pro_class=@pro_class ");
            SqlParameter[] parameters = {
        ...
        public bool Exists(string pro_class, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from  tbl_sinkia_class");
            strSql.Append(" where  id<>@id and  pro_class=@pro_class ");
            SqlParameter[] parameters = {
                     new SqlParameter("@id", SqlDbType.Int,4),
					new SqlParameter("@pro_class", SqlDbType.NVarChar,100)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into [tbl_sinkia_class] (");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [tbl_sinkia_class] set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tbl_sinkia_class set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from [tbl_sinkia_class] ");
            strSql.Append(" where id=@id ");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  tbl_sinkia_class");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
