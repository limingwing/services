...
public partial class manager_system_manage : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
           ...
           RptBind("role_id>" + 0+ CombSqlTxt(this.status,this.role_id, this.keywords), "add_time desc,id desc");
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("manage.aspx", "status={0}&category_id={1}&depot_id={2}&keywords={3}&role_id={4}&page={5}", this.status.ToString(), this.category_id.ToString(), this.depot_id.ToString(), this.keywords, this.role_id.ToString(), "__id__");
        PageContent.InnerHtml = Utils.OutPageList(this.pageSize, this.page, this.totalCount, pageUrl, 8);
    }
    #endregion

    #region 组合SQL查询语句==========================
    protected string CombSqlTxt(int _status,  int _role_id, string _keywords)
    {
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("manage.aspx", "status={0}&category_id={1}&depot_id={2}&keywords={3}&role_id={4}", this.status.ToString(), this.category_id.ToString(), this.depot_id.ToString(), txtKeywords.Text, this.role_id.ToString()));
    }
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("manage.aspx", "status={0}&category_id={1}&depot_id={2}&keywords={3}&role_id={4}",
    ...
    protected void lbtnDelCa_Click(object sender, EventArgs e)
    {
        ...
        if (caId == 1)
        {
            mym.JscriptMsg(this.Page, "对不起，最高管理员不能删除，请检查！", "", "Error");
            return;
        }
        tbl_admin bll = new tbl_admin();
        bll.GetModel(caId);
        ...
        mym.JscriptMsg(this.Page, " 成功删除用户名（账号）：" + title + "", Utils.CombUrlTxt("manage.aspx", "status={0}&category_id={1}&depot_id={2}&keywords={3}&page={4}", this.status.ToString(), this.category_id.ToString(), this.depot_id.ToString(), this.keywords, this.page.ToString()), "Success");
