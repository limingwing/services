...
public partial class manager_member_al_read6 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            RptBind("huiyuan_id<>'A00000000' and huiyuan_jiebie=0 and net_hege=1 " + CombSqlTxt(this.jiebie, this.note_no, this.start_time, this.stop_time,this.cha_type), "huiyuan_add_time desc");
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}&page={4}&type={5}", this.start_time.ToString(), this.stop_time.ToString(), this.jiebie.ToString(), this.note_no, "__id__", this.cha_type.ToString());
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("al_read6_page_size"), out _pagesize))
        {
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}&type={4}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.type.SelectedValue));
    }

    //筛选会员级别
    protected void ddlproduct_category_id_SelectedIndexChanged(object sender, EventArgs e)
    {
        Response.Redirect(Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text));
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
        {
            ...
            if (_pagesize > 0)
            {
                Utils.WriteCookie("al_read6_page_size", _pagesize.ToString(), 14400);
            }
        }
        Response.Redirect(Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}&type={4}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.type.SelectedValue));
    }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
      ...
      //  Response.Redirect(Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.jiebie.ToString(), txtNote_no.Text));
        StringBuilder strTemp = new StringBuilder();
        ...
        if (dt.Rows.Count == 0)
        {
            Response.Write("<script>alert('导出数据无效'); location.href='al_read6.aspx';</script>");
            return;
        }
        string filename = "";
    ...
    protected void lbtnAcceptCaDG_Click(object sender, EventArgs e)
    {
        ...
        mym.JscriptMsg(this.Page, " 设置成功！", Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}&type={4}&page={5}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.type.SelectedValue, this.page.ToString()), "Success");
    ...
    protected void lbtnAcceptCaDG_Click2(object sender, EventArgs e)
    {
        ...
        mym.JscriptMsg(this.Page, " 修改成功！", Utils.CombUrlTxt("al_read6.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}&type={4}&page={5}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.type.SelectedValue, this.page.ToString()), "Success");
