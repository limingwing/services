...
public partial class manager_member_al_read3 : System.Web.UI.Page
{
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("al_read3_page_size"), out _pagesize))
        {
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("al_read3.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.ddlproduct_category_id.SelectedValue, txtNote_no.Text));
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        //  Response.Redirect(Utils.CombUrlTxt("al_read3.aspx", "start_time={0}&stop_time={1}&huiyuan_jiebie={2}&note_no={3}", this.txtstart_time.Value, this.txtstop_time.Value, this.jiebie.ToString(), txtNote_no.Text));
        StringBuilder strTemp = new StringBuilder();
        ...
        if (dt.Rows.Count == 0)
        {
            Response.Write("<script>alert('导出数据无效'); location.href='al_read3.aspx';</script>");
            return;
        }
        string filename = "";
    ...
    protected string show_kongdan(int _kd_flag)
    {
        ...
        switch (_kd_flag)
        {
            case 0:
                ...
                break;
            case 1:
                _title = "<font color=red>一轮出局</font>";
                break;
