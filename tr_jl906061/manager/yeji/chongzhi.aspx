<%@ Page Language="C#" AutoEventWireup="true" CodeFile="chongzhi.aspx.cs" Inherits="manager_yeji_chongzhi" %>
...
<body>
    ...
    <table class="tablelist">
        ...
        <th width="10%">充值金额<font color="red"></th>
        <th width="8%">充值时间</th>
        <th width="20%">备注</th>
        <th width="5%">状态</th>
        <th width="8%">操作方法</th>
...
<ItemTemplate> 
     ...
     <asp:HiddenField ID="hidId" Value='<%#Eval("huikuan_id")%>' runat="server" /></td>
            <td><%# pageSize * page + Container.ItemIndex + 1 - pageSize%></td>
            <td><%# Eval("huiyuan_bianhao")%></td>

            <td><%# Eval("accountbank")%></td>
            <td><%# Eval("accountno")%></td>
            <td><%# Eval("accountname")%></td>
   
            <td> <%#Eval("money", "{0:f}")%></td>	
            <td><%#string.Format("{0:g}", Eval("hk_sq_time"))%></td>
            <td><%#Eval("memo") %></td>
            <td><%#GetOrderStatus(Convert.ToInt32(Eval("huikuan_status")))%>	</td>
            <td>
     <asp:LinkButton ID="lbtnModify" CommandName="zhifu" OnCommand="GetWhichCommand"  CommandArgument='<%# Eval("huikuan_id") %>'
                                    runat="server" OnClientClick='<%# String.Format("return confirm(\"您确定用户{0}充值{1:f}元信息吗？\")",Eval("huiyuan_bianhao"),Eval("money")) %>'
                                      Visible='<%# !Convert.ToBoolean(Convert.ToInt16(Eval("huikuan_status"))) %>'  ><font color =green>[确认]</font></asp:LinkButton>
                                    
                                    <asp:LinkButton ID="LinkButton1" CommandName="zhifu_no" OnCommand="GetWhichCommand"   CommandArgument='<%# Eval("huikuan_id")+","+Eval("huiyuan_id")+","+Eval("money")%>' 

                                    runat="server" OnClientClick='<%# String.Format("return confirm(\"您确定删除信息吗？\")",Eval("huiyuan_bianhao"),Eval("money")) %>'
                                     Visible='<%# !Convert.ToBoolean(Convert.ToInt16(Eval("huikuan_status"))) %>'  ><font color =red>[删除]</font></asp:LinkButton>
                                    </td>
